          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm text-blue-600">{stat.quizCount} ({safePct(stat.quizCount, totalStat?.totalQuizzes ?? stat.total).toFixed(1)}%)</span>
              </div>
              <div className="w-full bg-blue-200 rounded-full h-2">
                <div 
                  className="bg-blue-600 h-2 rounded-full transition-all duration-300" 
                  style={{ width: `${safePct(stat.quizCount, totalStat?.totalQuizzes ?? stat.total)}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm text-green-600">{stat.keywordCount} ({safePct(stat.keywordCount, totalStat?.totalKeywords ?? stat.total).toFixed(1)}%)</span>
              </div>
              <div className="w-full bg-green-200 rounded-full h-2">
                <div 
                  className="bg-green-600 h-2 rounded-full transition-all duration-300" 
                  style={{ width: `${safePct(stat.keywordCount, totalStat?.totalKeywords ?? stat.total)}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm text-red-600">{stat.reportCount} ({safePct(stat.reportCount, totalStat?.totalPendingReports ?? stat.total).toFixed(1)}%)</span>
              </div>
              <div className="w-full bg-red-200 rounded-full h-2">
                <div 
                  className="bg-red-600 h-2 rounded-full transition-all duration-300" 
                  style={{ width: `${safePct(stat.reportCount, totalStat?.totalPendingReports ?? stat.total)}%` }}
                ></div>
              </div>
            </div>
          </div>
